...
class MetaChatService:
    ...
    async def process_request(self, request: MetaChatRequest) -> MetaChatResponse:
        ...
        try:
            ...
            if intent.response_type == ResponseType.DIRECT or not intent.needs_agent:
                # Generate direct response
                logger.info("Generating direct response")
                response_data = await self._generate_direct_response(request.message, intent)
    ...
    async def _find_suitable_agent(self, intent: ChatIntent, original_message: str) -> Optional[Agent]:
        ...
        if response:
            try:
                ...
                if selected_agent and selected_agent.lower() != "none":
                    agent_name = selected_agent
                    
                    # Find the agent by name
                    for agent in agents:
                        if agent.name.lower() == agent_name.lower():
                            logger.info(f"LLM selected agent: {agent.name}")
                            return agent
                    
                    # If exact match not found, try partial match
                    for agent in agents:
                        if agent_name.lower() in agent.name.lower() or agent.name.lower() in agent_name.lower():
                            logger.info(f"LLM selected agent (partial match): {agent.name}")
                            return agent
            except Exception as e:
    ...
    async def _create_agent_for_intent(self, intent: ChatIntent, original_message: str) -> Optional[Agent]:
        ...
        try:
            ...
            requirement_desc = f"""Create an agent to handle this user request: "{original_message}"
            
The agent should be able to fulfill this specific need completely.
If it needs to access system information, create appropriate tools.
If it needs external data, create the necessary integrations."""
            
            # Generate a simple name based on the intent
